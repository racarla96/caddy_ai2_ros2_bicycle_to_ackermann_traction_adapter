...
def generate_launch_description():
    ...
    robot_controllers2 = PathJoinSubstitution(
        [
            FindPackageShare('bicycle_to_ackermann_traction_adapter'),
            'config',
            'bicycle_to_ackermann_traction_adapter.yaml',
        ]
    )
    ...
    ld = LaunchDescription([
        ...
        RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[forward_position_command_controller_spawner, ackermann_traction_controller_spawner],
            )
        ),
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=ackermann_traction_controller_spawner,
                on_exit=[forward_velocity_command_controller_spawner],
            )
        ),
        gz_spawn_entity,
        # Launch Arguments
